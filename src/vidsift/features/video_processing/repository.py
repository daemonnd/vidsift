...
class VideoProcessingRepository:
    def __init__(self, config: AppConfig, db_path: Path = PROCESSED_VIDEOS_DB) -> None:
    ...
    def save_validation_result(
        ...
        try:
            ...
            raise DBWritingError(
                f"Failed to write to DB while updating the status after validation because a database operand violated a constraint: {str(e)}"
            ) from e
        except OperationalError as e:
            raise DBWritingError(
    ...
    def update_after_done(
        self, video_id: str, decision: Literal["downloaded", "summarized", "discarded"]
    ):
        """
        ...
        try:
            ...
            raise DBWritingError(
                f"Failed to write to DB while updating the status after the video with id {video_id} has been {decision}, because a database operand violated a constraint: {str(e)}"
            ) from e
        except OperationalError as e:
            raise DBWritingError(
                f"Failed to write to DB while updating the status after the video with id {video_id} has been {decision}, because of an operational Error: {str(e)}"
            ) from e
        ...
        try:
            if not self.exists(video_id):
                raise VideoIDNotFoundError(
                    f"There is no row with the video id {video_id} in the database, it cannot be removed"
                )
            ...
            raise DBWritingError(
                f"Failed to write to DB while deleting row with video id {video_id} because a database operand violated a constraint: {str(e)}"
            )
        except OperationalError as e:
            raise DBWritingError(
                f"Failed to write to DB while deleting row with video id {video_id} because of an operational Error: {str(e)}"
            ) from e
