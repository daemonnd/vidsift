...
"""
        ...
        subprocess.run("systemctl --user daemon-reload && systemctl --user enable vidsift.service", shell=True)

    def uninstall_service(self) -> None:
        subprocess.run("systemctl --user daemon-reload && systemctl --user disable --now vidsift.service", shell=True)
