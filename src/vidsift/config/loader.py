...
def load_config(config_path: Path = VIDSIFT_CONFIG_FILE_PATH) -> AppConfig:
    try:
        ...
        raise ConfigFileNotFoundError(
            f"The config file has not been found at {config_path}: {str(e)}. You may need to run `vidsift init` for getting the default config"
        ) from e
