...
class ConsoleDependencyFilter(Filter):
    def __init__(
        self, console_config, name: str = "", debug_mode: bool = False
    ) -> None:
        super().__init__(name)
    ...
    def filter(self, record: LogRecord) -> bool | LogRecord:
        ...
        For vidsift logs
        """
        # if it is only for files, not for console
        ...
        if record.name.startswith("vidsift") or record.name == "__main__":
            ...
            if (
                record.levelno
                < logging.getLevelNamesMapping()[self.console_config.level]
            ):
                return False
            else:
                return True
        ...
        if (
            record.levelno
            >= logging.getLevelNamesMapping()[self.console_config.dependency_level]
        ):
            return True
        return False
...
class FileDependencyFilter(Filter):
    ...
    def filter(self, record: LogRecord) -> bool | LogRecord:
        ...
        if (
            record.levelno
            >= logging.getLevelNamesMapping()[self.file_config.dependency_level]
        ):
            return True
