    ...
    def get_transcript(self, video: Video) -> str:
        for provider in self.providers:
            try:
                ...
                logger.warning(f"TranscriptError: Failed to fetch the transcript with the current provider {provider.get_provider_name()}: {str(e)}")
            except Exception as e:
                logger.warning(f"Exception: Failed to fetch the transcript with the current provider {provider.get_provider_name()}: {str(e)}")
